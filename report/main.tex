...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.85\textheight, keepaspectratio]{er_diagram.png} 
...
\begin{itemize}
    ...
    The data shows a clear, classic bi-modal distribution corresponding to daily commuting patterns. Accidents peak sharply during the morning rush hour (7:00 AM -- 8:00 AM), reaching up to 587k incidents at 7:00 AM. A second, equally massive peak occurs during the evening commute (3:00 PM -- 6:00 PM), peaking at 582k incidents at 5:00 PM. This confirms that traffic volume is the absolute primary driver of accident frequency.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Best parameters found: \texttt{regParam = 10$^{-2}$}, \texttt{elasticNetParam = 0.0}.
    \end{itemize}
    \item \textbf{Random Forest Classifier}
    \begin{itemize}
        \item Hyperparameters tuned via ParamGrid: \texttt{numTrees} ($[40, 80, 120]$), \texttt{maxDepth} ($[6, 10]$).
        \item Best parameters found: \texttt{numTrees = 40}, \texttt{maxDepth = 10}.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Predictions saved:} JSON exports under \texttt{project/data/train|test} store \texttt{features} and \texttt{label} for the best RF pipeline; per-model test predictions (\texttt{label}, \texttt{prediction}) are CSVs under \texttt{project/output/}; aggregated metrics are in \texttt{project/output/evaluation}.
...
\subsection*{Outcomes}
The Stage 3 successfully demonstrated the end-to-end process of building predictive models for traffic accident severity using PySpark MLlib. Key achievements include:

\begin{itemize}
    \item Implementation of advanced feature engineering, including \texttt{CyclicalTimestampTransformer} for temporal data and \texttt{LatLngToEcefTransformer} for geospatial coordinates. The pipeline managed 41 features and addressed the 79.67\% class imbalance by optimizing for the Weighted F1-score.
    
    \item  Rigorous hyperparameter tuning of Multinomial Logistic Regression and Random Forest models using 3-fold cross-validation on a distributed YARN cluster.
    
    \item Identification of Random Forest as the superior model, achieving an Accuracy of 81.86\% and a Weighted F1-score of 76.72\% on the test set.
    
    \item All project requirements for this stage were met, including the use of PySpark ML on a YARN cluster, data sourcing from Hive, HDFS storage for models and outputs, and adherence to specified output formats. The process was automated via scripts (stage3.sh) and code quality was maintained (using pylint).
\end{itemize}
...
\subsection*{Tab 3: Severity Classification}
...
\begin{itemize}
    ...
    \item Cluster ingested timezones incorrectly the first time, had to specify timezone in Hive
\end{itemize}
...
\begin{tabularx}{\linewidth}{@{} >{\raggedright\arraybackslash}p{2cm} >{\raggedright\arraybackslash}X cccc >{\raggedright\arraybackslash}X c @{}}
  ...
  \textbf{Stage III:} Spark ML
    ...
    & 0\% & 0\% & 100\% & 0\%
    & \texttt{train\_ml.py}, \texttt{models/}, \texttt{evaluation.csv}, \texttt{stage3.sh}, \texttt{scripts/ml/transformers.py}, \texttt{output/model1\_predictions.csv}, \texttt{output/model2\_predictions.csv}
    & 18 \\
